    ...
    IN: Copy + Into<OUT>,
{
    ...
    for i in 0..outlength {
        toptr[i * 2] = OUT::default();
        toptr[i * 2 + 1] = OUT::default();
    }
...
pub fn reduce_sum_complex64_complex64_64(
    toptr: &mut [f32],
    fromptr: &[f32],
    parents: &[i64],
    outlength: usize,
) {
    reduce_sum_complex(toptr, fromptr, parents, outlength)
}
pub fn reduce_sum_complex128_complex128_64(
    toptr: &mut [f64],
    fromptr: &[f64],
    parents: &[i64],
    outlength: usize,
...
pub fn reduce_argmin_complex<IN>(
    ...
    IN: PartialOrd + Copy,
{
    for v in toptr[..outlength].iter_mut() {
        *v = -1;
    }
    for i in 0..parents.len() {
        let p = parents[i] as usize;
        ...
        if cur == -1 {
            ...
            if re_i < re_min || (re_i == re_min && im_i < im_min) {
                toptr[p] = i as i64;
            }
        }
    }
}
pub fn reduce_argmin_complex64_64(
    toptr: &mut [i64],
    fromptr: &[f32],
    parents: &[i64],
    outlength: usize,
) {
    reduce_argmin_complex(toptr, fromptr, parents, outlength)
}
pub fn reduce_argmin_complex128_64(
    toptr: &mut [i64],
    fromptr: &[f64],
    parents: &[i64],
    outlength: usize,
...
pub fn reduce_argmax_complex<IN>(
    toptr: &mut [i64],
    fromptr: &[IN],
    parents: &[i64],
    outlength: usize,
    ...
    IN: PartialOrd + Copy,
{
    ...
    for i in 0..parents.len() {
        ...
        if cur == -1 {
            ...
            if re_i > re_max || (re_i == re_max && im_i > im_max) {
                toptr[p] = i as i64;
            }
        }
    }
}
pub fn reduce_argmax_complex64_64(
    toptr: &mut [i64],
    fromptr: &[f32],
    parents: &[i64],
    outlength: usize,
) {
    reduce_argmax_complex(toptr, fromptr, parents, outlength)
}
pub fn reduce_argmax_complex128_64(
    toptr: &mut [i64],
    fromptr: &[f64],
    parents: &[i64],
    outlength: usize,
...
pub fn reduce_countnonzero_complex<IN>(
    toptr: &mut [i64],
    fromptr: &[IN],
    parents: &[i64],
    outlength: usize,
    ...
    IN: PartialEq + Default + Copy,
{
    ...
    let zero = IN::default();
    for i in 0..parents.len() {
        if fromptr[i * 2] != zero || fromptr[i * 2 + 1] != zero {
            toptr[parents[i] as usize] += 1;
        }
    }
}
pub fn reduce_countnonzero_complex64_64(
    toptr: &mut [i64],
    fromptr: &[f32],
    parents: &[i64],
    outlength: usize,
    ...
    for i in 0..parents.len() {
        if fromptr[i * 2] != 0.0 || fromptr[i * 2 + 1] != 0.0 {
            toptr[parents[i] as usize] += 1;
        }
    }
}
pub fn reduce_countnonzero_complex128_64(
    toptr: &mut [i64],
    fromptr: &[f64],
    parents: &[i64],
    outlength: usize,
...
pub fn reduce_max_complex<T>(
    ...
    T: PartialOrd + Copy + DefaultZero,
{
    for i in 0..outlength {
        toptr[i * 2] = identity;
        toptr[i * 2 + 1] = T::default_zero();
    }
    for i in 0..parents.len() {
        let p = parents[i] as usize;
        let x = fromptr[i * 2];
        let y = fromptr[i * 2 + 1];
        if x > toptr[p * 2] || (x == toptr[p * 2] && y > toptr[p * 2 + 1]) {
            toptr[p * 2] = x;
            toptr[p * 2 + 1] = y;
        }
    }
}
pub fn reduce_min_complex<T>(
    toptr: &mut [T],
    fromptr: &[T],
    parents: &[i64],
    outlength: usize,
...
impl DefaultZero for f64 {
    fn default_zero() -> Self {
        0.0
    }
}

pub fn reduce_max_complex64_complex64_64(
    toptr: &mut [f32],
    fromptr: &[f32],
    parents: &[i64],
    outlength: usize,
    ...
    reduce_max_complex(toptr, fromptr, parents, outlength, identity)
}
pub fn reduce_max_complex128_complex128_64(
    toptr: &mut [f64],
    fromptr: &[f64],
    parents: &[i64],
    outlength: usize,
...
pub fn reduce_min_complex64_complex64_64(
    toptr: &mut [f32],
    fromptr: &[f32],
    parents: &[i64],
    outlength: usize,
    ...
    reduce_min_complex(toptr, fromptr, parents, outlength, identity)
}
pub fn reduce_min_complex128_complex128_64(
    toptr: &mut [f64],
    fromptr: &[f64],
    parents: &[i64],
    outlength: usize,
...
pub fn reduce_prod_complex64_complex64_64(
    toptr: &mut [f32],
    fromptr: &[f32],
    parents: &[i64],
    outlength: usize,
...
pub fn reduce_prod_complex128_complex128_64(
    toptr: &mut [f64],
    fromptr: &[f64],
    parents: &[i64],
    outlength: usize,
...
pub fn reduce_prod_bool_complex<IN>(
    toptr: &mut [bool],
    fromptr: &[IN],
    parents: &[i64],
    outlength: usize,
...
pub fn reduce_prod_bool_complex64_64(
    toptr: &mut [bool],
    fromptr: &[f32],
    parents: &[i64],
    outlength: usize,
    ...
    for i in 0..parents.len() {
        toptr[parents[i] as usize] &= fromptr[i * 2] != 0.0 || fromptr[i * 2 + 1] != 0.0;
    }
}
pub fn reduce_prod_bool_complex128_64(
    toptr: &mut [bool],
    fromptr: &[f64],
    parents: &[i64],
    outlength: usize,
...
pub fn reduce_sum_bool_complex<IN>(
    toptr: &mut [bool],
    fromptr: &[IN],
    parents: &[i64],
    outlength: usize,
    ...
    IN: PartialEq + Default + Copy,
{
    ...
    for i in 0..parents.len() {
        if fromptr[i * 2] != zero || fromptr[i * 2 + 1] != zero {
            toptr[parents[i] as usize] = true;
        }
    }
}
pub fn reduce_sum_bool_complex64_64(
    toptr: &mut [bool],
    fromptr: &[f32],
    parents: &[i64],
    outlength: usize,
...
pub fn reduce_sum_bool_complex128_64(
    toptr: &mut [bool],
    fromptr: &[f64],
    parents: &[i64],
    outlength: usize,
