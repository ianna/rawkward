...
//     // bool, i64, f64 scalars (Cases 1-3) — unchanged
...
//                 shape: vec![len], strides: vec![1], dtype: DType::F64,
//             }));
//         }
//     }
...
//             contents.push(Arc::new(from_python_object_inner(&v, /*is_field_value=*/true)?));
//         }
//         return Ok(Content::RecordArray(crate::content::RecordArray {
//             fields, contents, length: 1,
//         }));
//     }

//     // Case 6: list → recurse (unchanged)
//     // ...
// }
// pub fn from_python_object(obj: &Bound<'_, PyAny>) -> PyResult<Content> {
        ...
        if let Ok(flat) = obj.extract::<Vec<i64>>() {
            ...
            return Ok(Content::ListOffsetArray(ListOffsetArray {
                ...
                content: Arc::new(Content::I64(NumpyArray {
                    data: Arc::from(flat.into_boxed_slice()),
                    shape: vec![len], strides: vec![1], dtype: DType::I64,
                })),
            }));
        }
        if let Ok(flat) = obj.extract::<Vec<f64>>() {
...
// Top-level: called from from_python_object
// fn merge_contents(children: Vec<Content>) -> PyResult<Content> {
//     // When all children are single-row RecordArrays from dicts, merge directly.
//     if children.iter().all(|c| matches!(c, Content::RecordArray(_))) {
...
fn merge_contents_inner(mut children: Vec<Content>) -> PyResult<Content> {
    if children.is_empty() {
        return Ok(Content::F64(NumpyArray {
            data: Arc::from([] as [f64; 0]),
            shape: vec![0], strides: vec![1], dtype: DType::F64,
        }));
    }
    ...
    if children.iter().all(|c| matches!(c, Content::Bool(_))) {
        let flat: Vec<bool> = children.iter().flat_map(|c| match c {
        ...
        return Ok(Content::Bool(NumpyArray {
            data: Arc::from(flat.into_boxed_slice()),
            shape: vec![len], strides: vec![1], dtype: DType::Bool,
        }));
    }

    if children.iter().all(|c| matches!(c, Content::I64(_))) {
        let flat: Vec<i64> = children.iter().flat_map(|c| match c {
    ...
    //             let inner = Arc::try_unwrap(a.content)
    ...
    //     }
    //     let merged_inner = merge_contents_inner(inner_items)?;
    //     return Ok(Content::ListOffsetArray(ListOffsetArray {
    //         offsets: Arc::from(merged_offsets.into_boxed_slice()),
    //         content: Arc::new(merged_inner),
    //     }));
...
// New — same but routes through merge_contents_inner, skipping the record shortcut.
...
// // Inner: called recursively — never applies the record shortcut
// fn merge_contents_inner(mut children: Vec<Content>) -> PyResult<Content> {
//     if children.is_empty() {
//         return Ok(Content::F64(NumpyArray {
//             data: Arc::from([] as [f64; 0]),
...
//     // RecordArray: merge column-wise (no shortcut — always produces RecordArray)
//     if children.iter().all(|c| matches!(c, Content::RecordArray(_))) {
//         return merge_record_children(children);
//     }

//     // ListOffsetArray: re-expand and recurse with merge_contents_inner (not merge_contents)
//     if children.iter().all(|c| matches!(c, Content::ListOffsetArray(_))) {
//         let mut merged_offsets: Vec<i64> = vec![0];
//         let mut inner_items: Vec<Content> = Vec::new();
//         for child in children.drain(..) {
...
//             merged_offsets.push(merged_offsets.last().unwrap() + item.len() as i64);
//         }
//         // Use merge_contents_inner recursively — no record shortcut
//         let merged_inner = merge_contents_inner(inner_items)?;
//         return Ok(Content::ListOffsetArray(ListOffsetArray {
//             offsets: Arc::from(merged_offsets.into_boxed_slice()),
//             content: Arc::new(merged_inner),
...
fn merge_record_children(mut children: Vec<Content>) -> PyResult<Content> {
    ...
    let total_rows: usize = children.iter().map(|c| match c {
        ...
        Content::IndexedOptionArray(a) => {
            let items: Vec<PyObject> = (0..a.len())
                .map(|i| match a.get(i as i64) {
                    ...
                    OptionValue::Some(c) => scalar_or_list(&c, py),  // unwrap scalars
                })
                .collect::<PyResult<_>>()?;
